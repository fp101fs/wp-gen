...
const getModelDisplayName = (modelId) => {
  const modelNames = {
    ...
    // Legacy model IDs for backward compatibility with existing database records
...
const getModelCreditCost = (modelId) => {
  const modelCosts = {
    'gemini-3-flash': 1,       // baseline
    'gemini-3-pro': 15,        // ~17x Flash pricing
    'claude-sonnet-4-5': 50,   // ~48x Flash pricing
    'claude-opus-4-6': 240,    // ~240x Flash pricing
    // Legacy model IDs
    'gemini-flash': 1,
    'gemini-pro': 15,
    'claude-opus': 240,
...
const executeAIGeneration = async (currentPrompt, revisionPrompt, parentExtension, uploadedImage, selectedProvider = 'claude-sonnet-4-5', platform = 'wordpress', onStatusUpdate = null) => {
  ...
  // Check if primary provider has API key (free tier models use server-side key)
  if (!isFreeTierModel) {
    if (primaryProvider === 'gemini-3-flash' && !hasGeminiFlashKey) {
      throw new Error('OpenRouter Flash API key not found');
    }
    if (primaryProvider === 'gemini-3-pro' && !hasGeminiProKey) {
      throw new Error('OpenRouter Pro API key not found');
    }
    if ((primaryProvider === 'claude-sonnet-4-5' || primaryProvider === 'claude-opus-4-6') && !hasClaudeKey) {
  ...
  if (primaryProvider === 'claude-opus-4-6') {
    if (hasClaudeKey) providersToTry.push('claude-sonnet-4-5');
    if (hasGeminiProKey) providersToTry.push('gemini-3-pro');
    if (hasGeminiFlashKey) providersToTry.push('gemini-3-flash');
  } else if (primaryProvider === 'claude-sonnet-4-5') {
    if (hasGeminiProKey) providersToTry.push('gemini-3-pro');
    if (hasGeminiFlashKey) providersToTry.push('gemini-3-flash');
  } else if (primaryProvider === 'gemini-3-pro') {
    if (hasClaudeKey) providersToTry.push('claude-sonnet-4-5');
    if (hasGeminiFlashKey) providersToTry.push('gemini-3-flash');
  } else if (primaryProvider === 'gemini-3-flash') {
    if (hasClaudeKey) providersToTry.push('claude-sonnet-4-5');
    if (hasGeminiProKey) providersToTry.push('gemini-3-pro');
  } else if (isFreeTierModel) {
    // Free models: cycle through other free models as fallback
    providersToTry.push(...FREE_MODELS.filter(m => m !== primaryProvider));
  }
      ...
      try {
        ...
        if (uploadedImage) {
          try {
            ...
            const maxSize = (currentProvider === 'claude-sonnet-4-5' || currentProvider === 'claude-opus-4-6') ? 5 * 1024 * 1024 : 20 * 1024 * 1024;
...
IMPORTANT: Response must be valid JSON only. The "files" object should contain all necessary files to make the plugin work. Keep file contents concise to avoid truncation.`;
        ...
        if (currentProvider === 'gemini-3-pro' || currentProvider === 'gemini-3-flash') {
          const OpenAI = (await import('openai')).default;

          const apiKey = currentProvider === 'gemini-3-flash'
            ? process.env.REACT_APP_OPENROUTER_FLASH_KEY
            : process.env.REACT_APP_OPENROUTER_PRO_KEY;

          const modelId = currentProvider === 'gemini-3-flash'
          ...
          debugLog(`Making API call to ${currentProvider === 'gemini-3-flash' ? 'Gemini 3 Flash' : 'Gemini 3 Pro'} via OpenRouter...`);
          ...
          const completion = await client.chat.completions.create({
            ...
            max_tokens: currentProvider === 'gemini-3-pro' ? 64000 : 32000
          });

          result = completion;
          apiResult = completion;
          debugLog(`Received result object from ${currentProvider === 'gemini-3-flash' ? 'Gemini 3 Flash' : 'Gemini 3 Pro'} via OpenRouter:`, result);

        } else if (FREE_MODELS.includes(currentProvider)) {
          // Free tier models via Vercel backend (server-side API key)
          const userContent = [];
          if (imageData) {
            userContent.push({ type: 'image_url', image_url: { url: `data:${imageData.mimeType};base64,${imageData.base64}` } });
          }
          userContent.push({ type: 'text', text: aiPrompt });

          debugLog(`Making API call to OpenRouter free (${currentProvider}) via backend...`);
          const freeFetchResponse = await fetch('/api/generate-openrouter-free', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              model: currentProvider,
              messages: [
                { role: 'system', content: 'You are an expert WordPress Developer. Generate high-quality, secure WordPress plugins. You respond only with valid JSON objects as specified.' },
                { role: 'user', content: imageData ? userContent : aiPrompt },
              ],
              max_tokens: 8192,
              temperature: 0.7,
            }),
          });
          if (!freeFetchResponse.ok) {
            const errData = await freeFetchResponse.json().catch(() => ({}));
            throw new Error(errData.error || `OpenRouter free request failed (${freeFetchResponse.status})`);
          }
          result = await freeFetchResponse.json();
          apiResult = result;
          debugLog(`Received result from OpenRouter free (${currentProvider}):`, result);

        } else if (currentProvider === 'claude-sonnet-4-5' || currentProvider === 'claude-opus-4-6') {
          ...
          const isOpus = currentProvider === 'claude-opus-4-6';
        ...
        let text;
        if (currentProvider === 'claude-sonnet-4-5' || currentProvider === 'claude-opus-4-6') {
          // Claude response processing
          const modelName = currentProvider === 'claude-opus-4-6' ? 'Claude Opus 4.6' : 'Claude Sonnet 4.5';
...
function HomePage({ session, sessionLoading, onShowLoginModal, isRevisionModalOpen, setIsRevisionModalOpen, extensionToRevise, setExtensionToRevise }) {
  ...
  const handleTokenizedRevision = async (revisionPrompt, parentExtension, selectedLLM = 'gemini') => {
    ...
    try {
      ...
      const tokenCost = selectedLLM === 'claude-opus-4-6' ? 240 : selectedLLM === 'claude-sonnet-4-5' ? 50 : selectedLLM === 'gemini-3-pro' ? 15 : 1;
